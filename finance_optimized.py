...
def process_ticker(ticker, batch_data):
...
def fetch_stock_data(tickers, max_workers=4):
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        # Fetch data for the batch once
        batch_data = yf.Tickers(" ".join(batch_tickers))
        
        # Process each ticker in parallel
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Create a partial function with the batch data
            process_func = partial(process_ticker, batch_data=batch_data)
            
            # Process tickers in parallel with progress bar
            results = list(tqdm(
                executor.map(process_func, batch_tickers),
                total=len(batch_tickers),
                desc="Processing tickers"
            ))
            
            # Add results to all_data
